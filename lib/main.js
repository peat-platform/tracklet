...
function emailCheck(params, callback) {
   ...
   request(url + arr.join('&'), function (error, response, body) {
      ...
      if (JSON.parse(body).value === true) {
         callback('email already exists: ' + params.email);
      } else {
         callback(null, params);
      }
...
function cloudletCheck(params, callback) {
   ...
   if(!params.name) {
      callback('user name not specified');
   } else {
      ...
      request(url, function (error, response, body) {
         ...
         if (_.includes(siteIds, params.cloudlet)) {
            callback('Cloudlet ID already exists: ' + params.cloudlet);
         } else {
            callback(null, params);
         }
      });
...
function createUser(params, callback) {
   ...
   arr.push('userLogin=' + params.name);
...
function deleteCheck(params, callback) {
   ...
   if(!params.name || !params.cloudlet) {
      callback('missing parameters');
   } else {

      async.parallel([
            async.apply(nameCheck, params),
...
function nameCheck(params, callback) {
   ...
   request(url + arr.join('&'), function (error, response, body) {
      ...
      if (JSON.parse(body).value === false) {
         callback('user name does not exist: ' + params.name);
      } else {
         callback(null, params);
      }
...
function cloudletCheck2(params, callback) {
   ...
   if(!params.name) {
      ...
      request(url, function (error, response, body) {
         ...
         if (!_.includes(siteIds, params.cloudlet)) {
            callback('Cloudlet ID does not exist: ' + params.cloudlet);
         } else {
            callback(null, params);
         }
      });
